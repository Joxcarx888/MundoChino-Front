...
export const InvoicesPage = () => {
  ...
  const handleOpenModal = (invoice = null) => {
    if (invoice) {
      ...
      // Convierte la fecha del backend a formato YYYY-MM-DD sin tocar la zona horaria
      const fecha = invoice.fechaCompra.split("T")[0];
  ...
  const filteredInvoices = useMemo(() => {
    return invoices.filter((inv) => {
      ...
      // Filtrado por fechas
      const invoiceDate = inv.fechaCompra.split("T")[0]; // YYYY-MM-DD
      const matchStart = startDate ? invoiceDate >= startDate : true;
      const matchEnd = endDate ? invoiceDate <= endDate : true;

      return matchNo && matchSerie && matchProv && matchProd && matchStart && matchEnd;
    });
